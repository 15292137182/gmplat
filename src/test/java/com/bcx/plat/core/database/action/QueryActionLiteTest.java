...
public class QueryActionLiteTest extends BaseTest {
    ...
    public void smogTest(){
        QueryAction queryActionLite=new QueryAction().select(fieldId, fieldDemo1, fieldDemo2).from(new Table("test_only.test_table1"))
    ...
    public void selectAll(){
        QueryAction queryActionLite=new QueryAction().selectAll().from(new Table("test_only.test_table1"))
                .where(new FieldCondition(fieldId, Operator.IN, Arrays.asList(1,2,3)));
