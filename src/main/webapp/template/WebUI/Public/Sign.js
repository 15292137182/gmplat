...
function CallSign(signId ,callBack) {
    ...
    $.ajax({
        ...
        error: function () {
            ...
            ibcpLayer.ShowMsg('未找到相关签名的定义：' + signId);
        }
    });
}
...
function SignLoad(sdata) {
    ...
    if(signType == 0){
        signHtmlUrl = getRootPath() +  '/WebUI/Public/Signature.html';
        signDivIndex = ibcpLayer.ShowDiv(signHtmlUrl,signTitle, '400px', '400px',function () {
            LoadSingleSignData(sdata);
        });
    }
    else  {
        signHtmlUrl = getRootPath() +  '/WebUI/Public/DoubleSignature.html';
        signDivIndex = ibcpLayer.ShowDiv(signHtmlUrl,signTitle, '700px', '500px',function () {
 ...
 //获取签名组成员
    ...
    function FillSigGroupMember(member,userSelect){
        ...
        for (var i = 0; i < member.length; i++) {
         ...
         var ss = "";
         ss += "<option value='" + account + "'>" + name + "(" + account + ")</option>";
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (errCode == 0) {
                    ...
                    //alert(errCode);
                    ibcpLayer.ShowMsg(errCode);
                }
            },
            error: function () {
                //alert('签名服务异常！');
                ibcpLayer.ShowMsg('签名服务异常！');
            }
        });
