...
function CleverTabs(self, options) {
  ...
  this.options = $.extend({
    ...
    panelContainer: (function () {
      ...
      var panelElement = $('<div id="cleverTabsPanelContainer' + tick
          + '"></div>');
...
CleverTabs.prototype.add = function (options) {
  ...
  if (self.panelContainer.attr('id').indexOf('cleverTabsPanelContainer')
      === 0) {
    var height = self.wrapper.height() - self.element.height() - 30;
    self.panelContainer.css('height', Math.floor(height / self.wrapper.height()
        * 100) + '%');
  }
...
CleverTabs.prototype.getCurrentUniqueId = function () {
  ...
  if (self.element.find(' > li').size() > 0) {
    ...
    return current.size() > 0 ? CleverTab.getUniqueByHeaderId(
...
CleverTabs.prototype.getTabByUrl = function (url) {
  ...
  var self = this;
  var frames = self.panelContainer.find(
      'div[id^="cleverTabPanelItem-"]>iframe');
  var tab;
  for (i = 0; i < frames.size(); i++) {
    ...
    if (src == url.toLowerCase()) {
      tab = new CleverTab(self,
          CleverTab.getUniqueByPanelId(frame.parent('div:first').attr('id')));
    }
  }
  return tab;
}
...
function CleverTab(tabs, id) {
  ...
  //Tab中iframe的url
  this.url = (this.panel ? this.panel.find(' > iframe:first').attr('src')
      : null);
  //Tab关闭时的回调函数
...
CleverTab.prototype.deactivate = function () {
  ...
  if (self.tabs.lockOnlyOne && !self.tabs.hashtable[self.id].orgLock
      && self.tabs.element.find('>li').size() > 1) {
    self.setLock(false);
  }
}
//使Tab页面处于激活状态
...
CleverTab.prototype.kill = function () {
  ...
  if (self.tabs.panelContainer.attr('id').indexOf('cleverTabsPanelContainer')
      === 0) {
    var height = self.tabs.wrapper.height() - self.tabs.element.height() - 30;
    self.tabs.panelContainer.css('height', Math.floor(height
        / self.tabs.wrapper.height() * 100) + '%');
  }
...
CleverTab.prototype.refresh = function () {
  ...
  if (self.url.indexOf('clever_tabs_time_stamp=') > 0) {
    self.url = self.url.substring(0, self.url.indexOf('clever_tabs_time_stamp=')
        - 1);
  }
  var newUrl = self.url.concat(self.url.indexOf('?') < 0 ? '?' : '&').concat(
      'clever_tabs_time_stamp=').concat(new Date().getTime());
...
CleverTab.prototype.setLock = function (lock, changeOrgLock) {
  ...
  if (!lock && self.tabs.lockOnlyOne && self.tabs.element.find('>li').size()
      == 1) {
    return;
  }
  if (!lock) {
    ...
    var btnClose = $(
        '<a href="javascript:void(0)" class="ui-corner-all" title="Close" style="position: absolute; cursor: pointer; padding: 0px; top: 1px; right: 1px"><span class="ui-icon ui-icon-close"></span></a>');
