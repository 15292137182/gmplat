...
layui.define(['layer', 'laypage', 'laytpl'], function (exports) {
  "use strict";
  var $ = layui.jquery,
      layer = parent.layui.layer === undefined ? layui.layer
          : parent.layui.layer,
      laytpl = layui.laytpl;
  ...
  Paging.prototype.init = function (options) {
    ...
    if (_config.type.toUpperCase() !== 'GET' && _config.type.toUpperCase()
        !== 'POST') {
  ...
  Paging.prototype.get = function (options) {
    ...
    $.ajax({
      ...
      success: function (result, status, xhr) {
        if (result.code === 0) {
          //获取模板
          var tpl = _config.tempType === 0 ? $(_config.tempElem).html()
              : _config.tempElem;
          //渲染数据
          ...
          if (_config.paged) {
            ...
            var pages = result.count % pageSize == 0 ? (result.count / pageSize)
