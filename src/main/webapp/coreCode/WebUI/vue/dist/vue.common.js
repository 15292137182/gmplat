...
// The coreCode compiler attempts to minimize the need for normalization by
// statically analyzing the coreCode at compile time.
//
...
// e.g. <coreCode>, <slot>, v-for, or when the children is provided by user
  ...
  if (!vm.$options.render) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if ((vm.$options.template && vm.$options.template.charAt(0) !== '#') ||
          vm.$options.el || el) {
        warn(
            'You are using the runtime-only build of Vue where the coreCode ' +
        ...
        warn(
            'Failed to mount component: coreCode or render function not defined.',
            vm
        );
      }
    }
...
function createComponentInstanceForVnode(vnode, // we know it's MountedComponentVNode but flow doesn't
  ...
  // check inline-coreCode render functions
...
function initRender(vm) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    defineReactive$$1(vm, '$listeners', vm.$options._parentListeners, null,
        true);
  }
}
...
var isHTMLTag = makeMap(
    ...
    'content,element,shadow,coreCode,blockquote,iframe,tfoot'
);
...
function genRadioModel(el,
  ...
  addHandler(el, CHECKBOX_RADIO_TOKEN, genAssignmentCode(value, valueBinding),
      null, true);
}
  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      var reStackedTag = reCache[stackedTag]
          || (reCache[stackedTag] = new RegExp('([\\s\\S]*?)(</' + stackedTag
              + '[^>]*>)', 'i'));
    ...
    if (html === last) {
      ...
      if (process.env.NODE_ENV !== 'production' && !stack.length
          && options.warn) {
        options.warn(
            ("Mal-formatted tag at end of coreCode: \"" + html + "\""));
      }
      break
    }
  }
    ...
    start: function start(tag, attrs, unary) {
      ...
      if (!root) {
        ...
        if (root.if && (element.elseif || element.else)) {
          ...
          warnOnce(
              "Component coreCode should contain exactly one root element. " +
    ...
    chars: function chars(text) {
      if (!currentParent) {
        if (process.env.NODE_ENV !== 'production') {
          if (text === template) {
            warnOnce(
                'Component coreCode requires a root element, rather than just text.'
            );
...
function processKey(el) {
  ...
  if (exp) {
    if (process.env.NODE_ENV !== 'production' && el.tag === 'template') {
      warn$2(
          "<coreCode> cannot be keyed. Place the key on real elements instead.");
    }
    el.key = exp;
  }
}
...
function processComponent(el) {
  ...
  if (getAndRemoveAttr(el, 'inline-coreCode') != null) {
...
// for script (e.g. type="x/coreCode") or style, do not decode content
...
/**
 * Goal of the optimizer: walk the generated coreCode AST tree
...
function markStatic$1(node) {
  ...
  if (node.type === 1) {
    ...
    if (
        ...
        node.tag !== 'slot' &&
        node.attrsMap['inline-coreCode'] == null
    ) {
      return
    }
...
function genInlineTemplate(el, state) {
  ...
  if (process.env.NODE_ENV !== 'production' && (
    ...
    state.warn(
        'Inline-coreCode components must have exactly one child element.');
  }
  if (ast.type === 1) {
...
// detect problematic expressions in a coreCode
...
function checkIdentifier(ident, type, text, errors) {
  if (typeof ident === 'string' && !identRE.test(ident)) {
    errors.push(("invalid " + type + " \"" + ident + "\" in expression: "
        + (text.trim())));
  }
}
      ...
      try {
        ...
        if (e.toString().match(/unsafe-eval|CSP/)) {
          warn(
              ...
              +
              'The coreCode compiler cannot work in this environment. Consider '
              +
...
Vue$3.prototype.$mount = function (el,
  ...
  // resolve coreCode/el and convert to render function
  if (!options.render) {
    ...
    if (template) {
      if (typeof template === 'string') {
        ...
        if (process.env.NODE_ENV !== 'production') {
          warn('invalid coreCode option:' + template, this);
        }
        return this
      }
    } else if (el) {
