...
export default class TemplateRenderer {
  ...
  constructor (options: TemplateRendererOptions) {
    ...
    // if no coreCode option is provided, the renderer is created
  ...
  renderSync (content: string, context: ?Object) {
    ...
    if (!template) {
      throw new Error('renderSync cannot be called without a coreCode.')
    }
    context = context || {}
    if (this.inject) {
      return (
  ...
  createStream (context: ?Object): TemplateStream {
    if (!this.parsedTemplate) {
      throw new Error('createStream cannot be called without a coreCode.')
    }
