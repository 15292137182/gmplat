...
if (jQuery) {
  (function ($) {
    ...
    function move(target, event, animate) {
      ...
      if (target.parent().is('.minicolors-slider-wheel') && picker.parent().is(
              '.minicolors-grid')) {
        wx = 75 - x;
        wy = 75 - y;
      ...
      if (target.is('.minicolors-grid, .minicolors-slider')) {
        ...
        switch (settings.control) {
          ...
          case 'saturation':
            ...
            minicolors.find('.minicolors-grid-inner').css('opacity', saturation
                / 100);
            break;
          ...
          case 'brightness':
            ...
            slider.css('backgroundColor',
                hsb2hex({h: hue, s: saturation, b: 100}));
            minicolors.find('.minicolors-grid-inner').css('opacity', 1
                - (brightness / 100));
            break;
    ...
    function updateFromInput(input, preserveInputValue) {
      ...
      if (settings.opacity) {
        ...
        opacity = input.attr('data-opacity') === '' ? 1 : keepWithin(
            parseFloat(input.attr('data-opacity')).toFixed(2), 0, 1);
        if (isNaN(opacity)) {
          opacity = 1;
        }
        input.attr('data-opacity', opacity);
      ...
      switch (settings.control) {
        ...
        case 'brightness':
          ...
          // Update UI
          slider.css('backgroundColor', hsb2hex({h: hsb.h, s: hsb.s, b: 100}));
          minicolors.find('.minicolors-grid-inner').css('opacity', 1 - (hsb.b
              / 100));
          break;
    ...
    function doChange(input, hex, opacity) {
      ...
      if (!lastChange || lastChange.hex !== hex || lastChange.opacity
          !== opacity) {
