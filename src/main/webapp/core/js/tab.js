...
layui.define(['element', 'common'], function (exports) {
  ...
  Tab.prototype.init = function () {
    ...
    if (typeof(_config.elem) !== 'string' && typeof(_config.elem)
        !== 'object') {
  ...
  Tab.prototype.tabAdd = function (data) {
    ...
    if (tabIndex === -1) {
      ...
      if (_config.maxSetting !== 'undefined') {
        var currentTabCount = _config.elem.children(
            'ul.layui-tab-title').children('li').length;
        ...
        if (typeof _config.maxSetting === 'object') {
          ...
          var msg = _config.maxSetting.tipMsg || '为了系统的流畅度，只能同时打开' + max
              + '个选项卡。';
      ...
      //iframe 自适应
      ELEM.contentBox.find('iframe[data-id=' + globalTabIdIndex + ']').each(
          function () {
      ...
      if (_config.autoRefresh) {
        _config.elem.find('div.layui-tab-content > div').eq(tabIndex).children(
    ...
    if (_config.contextMenu) {
      ...
      ELEM.titleBox.find('li').on('contextmenu', function (e) {
        ...
        var $target = e.target.nodeName === 'LI' ? e.target
        ...
        if ($context.length > 0) {
          $context.eq(0).children('ul').children('li').each(function () {
            ...
            $that.on('click', function () {
              ...
              switch (target) {
                case 'refresh': //刷新当前
                  var src = ELEM.contentBox.find('iframe[data-id=' + id
                      + ']')[0].src;
                  ELEM.contentBox.find('iframe[data-id=' + id
                      + ']')[0].src = src;
                  break;
