        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (options.code) { //如果设置地区编码，则忽略单独设置的信息
              ...
              c = options.code - (options.code % 1e4 ? options.code % 1e2
                  : options.code);
              if (data[c]) {
                options.city = c;
              }
              ...
              for (code in data) {
                if (!(code % 1e4)) { //获取所有的省级行政单位
                  ...
                  if (options.required && !options.province) {
                    ...
                    options.province = isNaN(options.province) ? code
                        : options.province;
                  }
                } else {
                  ...
                  if (options.province && p > 0 && p < 1e4) { //同省的城市或地区
                    if (!(code % 100)) {
                      ...
                      if (options.required && !options.city) {
                        ...
                        options.city = isNaN(options.city) ? code
                            : options.city;
                      }
                      ...
                      if (options.city && c > 0 && c < 100) { //同个城市的地区
                        ...
                        if (options.required && !options.area) {
                          ...
                          options.area = isNaN(options.area) ? code
                              : options.area;
                        }
                      }
                    } else {
            ...
            var format = {
              province: function () {
                ...
                for (i in province) {
                  $province.append('<option value="' + i + '">' + province[i]
                      + '</option>');
                }
                if (options.province) {
              ...
              city: function () {
                ...
                for (i in city) {
                  $city.append('<option value="' + i + '">' + city[i]
                      + '</option>');
                }
                if (options.city) {
              ...
              area: function () {
                ...
                if (!hasCity) {
                  ...
                  for (i in area) {
                    $area.append('<option value="' + i + '">' + area[i]
                        + '</option>');
                  }
                  if (options.area) {
