...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
  ...
  BTable.prototype.render = function () {
    ...
    if (_config.checkbox && !_config.singleSelect) {
      th = '<th style="width:28px;"><input type="checkbox" lay-filter="allselector" lay-skin="primary" /></th>'
          + th;
    } else if (_config.checkbox) {
      th = '<th style="width:28px;">#</th>' + th;
    }
    var tpl = '<div class="btable">';
    if (_config.skin !== undefined && (_config.skin === 'line' || _config.skin
            === 'row' || _config.skin === 'nob') && _config.even) {
      tpl += '<table class="layui-table layui-form" lay-even lay-skin="'
          + _config.skin + '">';
    } else if (_config.skin !== undefined && (_config.skin === 'line'
            || _config.skin === 'row' || _config.skin === 'nob')) {
    ...
    paging.init({
      ...
      success: function () { //完成的回调
        ...
        form.on('checkbox(allselector)', function (data) {
          ...
          $(_config.elem).find('tbody.btable-content').children('tr').each(
              function () {
                var $that = $(this);
                //全选或反选
                $that.children('td').eq(0).children(
                    'input[type=checkbox]')[0].checked = elem.checked;
                form.render('checkbox');
              });
        });
        //绑定选择行事件
              ...
              $that.on('click', function () {
                ...
                if (_config.singleSelect) {
                  $that.siblings().each(function () {
                    $(this).children('td').eq(0).children(
                  ...
                  //获取当前的状态
                  var currState = $that.children('td').eq(0).children(
                      'input[type=checkbox]')[0].checked;
                  $that.children('td').eq(0).children(
                      'input[type=checkbox]')[0].checked = !currState;

                  //当前已选择的总行数
                  var cbxCount = 0;
                  $that.parent('tbody').children('tr').each(function () {
                    var $that = $(this);
                    if ($that.children('td:first-child').children(
                            'input')[0].checked) {
                      cbxCount++;
                    }
                  });
                  $(_config.elem).find('thead').children('tr').children(
                      'th:first-child').children(
                      'input[type=checkbox]')[0].checked =
  ...
  function getTpl(options) {
    ...
    if (options.checkbox) {
      tds = '<td><input type="checkbox" data-id="{{ item.' + options.field
          + ' }}" lay-skin="primary" /></td>' + tds;
    } else {
      tds = '<td style="display:none;"><input type="hidden" data-id="{{ item.id }}" name="id" /></td>'
          + tds;
    }
    tpl += '<tr>' + tds + '</tr>'
    tpl += '{{# }); }}';
    tpl += '{{# }else{ }}';
    var colLength = options.checkbox && !options.singleSelect ? columns.length
        + 1 : columns.length;
