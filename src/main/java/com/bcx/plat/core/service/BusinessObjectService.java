...
//            update(oldRowId);
      return null;//  ServerResult.Msg(BaseConstants.STATUS_SUCCESS, Message.UPDATE_SUCCESS);
    } else {
      return null;//  ServerResult.Msg(BaseConstants.STATUS_FAIL, Message.UPDATE_FAIL);
    }
  ...
  public ServerResult delete(String rowId) {
    ...
    for (FrontFunc busin : businObj) {
      ...
      if (funcRowId.size() != 0) {
        return null;// ServerResult.Msg(BaseConstants.STATUS_FAIL, Message.DATA_QUOTE);
      }
    }
    if (businObj.size() == 0) {
...
//                delete(args);
        return null;// ServerResult.Msg(BaseConstants.STATUS_SUCCESS, Message.DELETE_SUCCESS);
      }
    }
    return null;//  ServerResult.Msg(BaseConstants.STATUS_FAIL, Message.DATA_QUOTE);
